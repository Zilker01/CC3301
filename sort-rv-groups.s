...
.while_begin:           #     // del ciclo para mayor eficiencia
    ...
    lw      a0,0(t0)    #     int a0= p[0]; (num1)
    lw      a1,4(t0)    #     int a1= p[1]; (num2)
    li      t2,0        #     int t2= 0; (contador de grupos para p[0])
    li      t3,0        #     int t3= 0; (contador de grupos para p[1])
    li      a2,0        #     int bit_anterior = 0;
    li      t1,0        #     int t1= 0;

    .while1:
    beq     a0,zero,.bit_reset 
    andi    a3,a0,1
    beq     a2,zero,.if1
    j       .update1
